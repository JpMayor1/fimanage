...
interface UpdateExpenseCategoryI {
...
const UpdateExpenseCategory = ({
  category,
  onClose,
}: UpdateExpenseCategoryI) => {
  const { updateCategory, updateLoading } = useExpenseStore();

  // now structured like CreateExpenseCategory but for single item
  const [cat, setCat] = useState<{
    ...
    name: category.name,
    icon: category.icon as ExpenseIconKey,
    showIcons: false,
  });
  ...
  const handleIconChange = (value: ExpenseIconKey) =>
  ...
  return (
    <motion.div
      key="update-expense-category-modal"
      initial="initial"
      animate="animate"
      exit="exit"
        ...
        <form className="space-y-4 w-full" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center gap-2 relative">
            ...
            <div className="relative">
              ...
              >
                {(() => {
                  const Icon = expenseIcons[cat.icon];
              ...
              {cat.showIcons && (
                <div className="absolute mt-1 w-60 flex flex-wrap gap-2 p-2 bg-black border border-white/20 rounded-md shadow-lg z-40">
                  {Object.keys(expenseIcons).map((key) => {
                    const Icon = expenseIcons[key as ExpenseIconKey];
                    return (
                      <button
                        key={key}
                        type="button"
