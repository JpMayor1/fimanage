...
interface DeleteDeptI {
  dept: DeptType;
  onClose: () => void;
}

const DeleteDept = ({ dept, onClose }: DeleteDeptI) => {
  const { deleteDept, loading } = useDeptStore();

  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const success = await deleteDept(dept._id!);
    if (success) {
      onClose();
    }
  }
        ...
        >
          <div className="w-full">
            ...
            <p className="text-white text-center">
              ...
              <span className="text-red">{dept.lender}</span>?
            </p>
          </div>
