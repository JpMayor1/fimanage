...
interface GroupedInvestmentsProps {
  groupedInvestments: Record<string, InvestmentType[]>;
  onUpdate: (investment: InvestmentType) => void;
  ...
  return (
    <div className="space-y-4">
      {Object.keys(groupedInvestments)
        ...
        .map((date) => {
          const dailyInvestments = groupedInvestments[date];
          const isOpen = expandedDates.includes(date);
          const total = dailyInvestments.reduce((sum, e) => sum + e.amount, 0);
              ...
              >
                <div className="text-left">
                  ...
                  <p className="text-white/50 text-xs mt-0.5">
                    {dailyInvestments.length}{" "}
                    {dailyInvestments.length > 1 ? "investments" : "investment"}{" "}
                    • Total{" "}
