...
const DeleteSaving = ({ saving, onClose }: DeleteSavingI) => {
  const { deleteSaving, deleteLoading } = useSavingStore();

  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const success = await deleteSaving(saving._id!);
    if (success) {
      onClose();
    }
  }
