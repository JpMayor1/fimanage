...
interface GroupedSavingsProps {
  groupedSavings: Record<string, SavingType[]>;
  onUpdate: (saving: SavingType) => void;
  onDelete: (saving: SavingType) => void;
}
...
const GroupedSavings = ({
  ...
  return (
    <div className="space-y-4">
      {Object.keys(groupedSavings)
        .sort((a, b) => new Date(b).getTime() - new Date(a).getTime())
        .map((date) => {
          const dailySavings = groupedSavings[date];
          const isOpen = expandedDates.includes(date);
          const total = dailySavings.reduce((sum, e) => sum + e.amount, 0);
              ...
              >
                <div className="text-left">
                  ...
                  <p className="text-white/50 text-xs mt-0.5">
                    {dailySavings.length}{" "}
                    {dailySavings.length > 1 ? "savings" : "saving"} • Total{" "}
                ...
                {isOpen && (
                  ...
                  >
                    <div className="p-2 md:p-4 space-y-2">
                      {dailySavings.map((saving) => {
                        const Icon = savingIcons[saving.icon];
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                <p
                                  ...
                                  onMouseEnter={() =>
                                    !isMobile &&
                                    setActiveDescription(saving._id!)
                                  }
                                  onMouseLeave={() =>
