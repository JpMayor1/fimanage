...
interface DeleteSourceI {
  source: SourceType;
  onClose: () => void;
}

const DeleteSource = ({ source, onClose }: DeleteSourceI) => {
  const { deleteSource, loading } = useSourceStore();

  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const success = await deleteSource(source._id!);
    if (success) {
      onClose();
    }
  }
        ...
        >
          <div className="w-full">
            ...
            <p className="text-white text-center">
              ...
              <span className="text-red">{source.name}</span>?
            </p>
          </div>
