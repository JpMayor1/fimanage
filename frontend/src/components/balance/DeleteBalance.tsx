...
interface DeleteBalanceI {
  balance: BalanceType;
  onClose: () => void;
}

const DeleteBalance = ({ balance, onClose }: DeleteBalanceI) => {
  const { deleteBalance, deleteLoading } = useBalanceStore();

  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    const success = await deleteBalance(balance._id!);
    if (success) {
      onClose();
    }
  }
        ...
        >
          <div className="w-full">
            ...
            <p className="text-yellow text-center mt-1">
              {balance.name?.length > 20
                ? balance.name.substring(0, 20) + "..."
                : balance.name}
            </p>
          </div>
